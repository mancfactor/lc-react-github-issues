...
function App() {
  ...
  const { isSuccess: isSuccessOpen, data: issuesOpen } = useQuery(
    'issuesOpen',
    fetchIssuesOpen
  );

  const { isSuccess: isSuccessClosed, data: issuesClosed } = useQuery(
    'issuesClosed',
    fetchIssuesClosed
  );
  ...
  function fetchIssues() {
    return fetch(
      `https://api.github.com/repos/${repoName}/issues?per_page=10&state=${filter}`
    ).then(response => response.json());
  }
  function fetchIssuesOpen() {
    return fetch(
      `https://api.github.com/search/issues?q=repo:${repoName}+type:issue+state:open&per_page=1`
    ).then(response => response.json());
  }

  function fetchIssuesClosed() {
    return fetch(
      `https://api.github.com/search/issues?q=repo:${repoName}+type:issue+state:closed&per_page=1`
    ).then(response => response.json());
  }
      ...
      {isSuccess && (
        <div className="issues-container">
          <div className="issues-heading">
            <a href={`issue.repository_url`}>{repoName}</a>
            <div className="open-closed-buttons">
              <button onClick={() => setFilter('open')}>
                <IconOpen />
                <span className={filter === 'open' ? 'font-bold' : ''}>
                  {isSuccessOpen && <span>{issuesOpen.total_count} Open</span>}
                </span>
              </button>
